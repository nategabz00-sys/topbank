...
const recents = [
  ...
  { name: "Dad", tag: "ΓÇóΓÇó 8821", tint: "gradient-brand" },
  { name: "Jio", tag: "@jio", tint: "bg-accent text-accent-foreground" },
  { name: "Nia", tag: "@nia_p", tint: "gradient-emerald" },
  { name: "Rey", tag: "ΓÇóΓÇó 4410", tint: "gradient-brand" },
];

const accounts = [
  { id: "primary", label: "Primary ┬╖ ΓÇóΓÇó4271", balance: 184529.47 },
  { id: "savings", label: "Everyday ┬╖ ΓÇóΓÇó8802", balance: 42130.1 },
];

const methods = [
  { id: "insta", icon: "ΓÜí", title: "InstaSend", desc: "Free ┬╖ Arrives instantly" },
  { id: "bank", icon: "≡ƒÅª", title: "Bank transfer", desc: "Any Philippine bank ┬╖ Free" },
  { id: "intl", icon: "≡ƒîì", title: "International", desc: "120+ countries ┬╖ Low fee" },
  { id: "wallet", icon: "≡ƒô▒", title: "Mobile wallet", desc: "GCash, Maya, PayPal" },
] as const;
...
function Transfer() {
  ...
  return (
    <MobileShell>
      {/* Hero amount card ΓÇö primary focus */}
      <section className="px-5 pt-8">
        <div className="relative overflow-hidden rounded-[28px] gradient-card p-6 text-white shadow-elevated">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-semibold text-white/70">Γé▒</span>
            <input
              value={amount}
          ...
          {accounts.map((a) => (
            ...
            >
              ...
              <p className="mt-2 text-sm font-bold tabular-nums">
                Γé▒{a.balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </p>
            </button>
          ))}
        </div>
      ...
      <section className="mt-5 px-5">
        <div className="rounded-3xl border border-border bg-card p-4 shadow-card">
          <SummaryRow
            label="Amount"
            value={`Γé▒${amountNum.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
          />
          <SummaryRow
            label="Fee"
            value={
              fee === 0 ? "Free" : `Γé▒${fee.toLocaleString(undefined, { minimumFractionDigits: 2 })}`
            }
          />
          <div className="my-2 border-t border-border" />
          <SummaryRow
            label="Total"
            value={`Γé▒${total.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
            bold
          />
        </div>
        ...
        >
          {sending ? (
            ...
            <>
              <Zap className="h-4 w-4" strokeWidth={2.6} /> Send Γé▒{amountNum.toLocaleString()} now
            </>
          )}
        </button>
...
function SuccessModal({ receipt, onClose }: { receipt: TransferReceipt; onClose: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      {/* Flying money layer ΓÇö sits above backdrop, below modal */}
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="mt-5 text-4xl font-bold tabular-nums tracking-tight">
            Γé▒{receipt.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
          </p>
